...
CANDIDATES := 'linux-arm64' 'linux-armv6' 'linux-armv7' 'linux-ppc64le' 'linux-x64' 'linux-x86' 'windows-x64' 'windows-x86' 'darwin-x64' 'darwin-x86'
...
.PHONY: xgboost-native-darwin-%
xgboost-native-darwin-%:
	set -eux && \
	OS_ARCH=$(subst xgboost-native-,,$@) && \
	ARCH=$(subst xgboost-native-darwin-x,,$@) && \
	echo ${CANDIDATES} | grep -q $${OS_ARCH} && \
	cd ${HIVEMALL_HOME} && \
	docker run --rm -e CROSS_TRIPLE=darwin$${ARCH} \
		multiarch/crossbuild sh -c ' \
		apt-get update && \
